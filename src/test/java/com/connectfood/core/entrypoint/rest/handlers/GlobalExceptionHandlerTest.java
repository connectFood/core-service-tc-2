...
class GlobalExceptionHandlerTest {
  ...
  void shouldHandleNotFoundException() {
    ...
    Mockito.when(request.getRequestURI()).thenReturn("/restaurants-items/123");
  ...
  void shouldHandleBadRequestException() {
    final var request = Mockito.mock(HttpServletRequest.class);
    Mockito.when(request.getRequestURI()).thenReturn("/restaurants-items");
  ...
  void shouldHandleMethodArgumentNotValidException() {
    ...
    Assertions.assertEquals("name", body.errors().get(0).field());
    Assertions.assertEquals("Name is required", body.errors().get(0).message());
    Assertions.assertEquals("value", body.errors().get(1).field());
    Assertions.assertEquals("Value is required", body.errors().get(1).message());
  }
  ...
  void shouldHandleConstraintViolationException() {
    ...
    Mockito.when(path1.toString()).thenReturn("name");
    Mockito.when(path2.toString()).thenReturn("requestType");

    Mockito.when(violation1.getPropertyPath()).thenReturn(path1);
    Mockito.when(violation1.getMessage()).thenReturn("Name is required");

    Mockito.when(violation2.getPropertyPath()).thenReturn(path2);
    Mockito.when(violation2.getMessage()).thenReturn("Request Type is required");
    ...
    final var fields = body.errors().stream().map(FieldErrorResponse::field).toList();
    final var messages = body.errors().stream().map(FieldErrorResponse::message).toList();
