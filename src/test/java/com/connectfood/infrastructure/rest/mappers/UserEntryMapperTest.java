...
class UserEntryMapperTest {

  @Mock
  private UserTypeEntryMapper usersTypeMapper;
  ...
  void shouldReturnNullWhenOutputIsNull() {
    final UserResponse result = mapper.toResponse(null);
  ...
  void shouldConvertToResponseWhenUsersTypeAndAddressAreNull() {
    ...
    final UserResponse result = mapper.toResponse(output);
  ...
  void shouldConvertToResponseWhenUsersTypeIsPresentAndAddressIsNull() {
    ...
    Assertions.assertSame(userTypeResponse, result.getUsersType());
    Assertions.assertNull(result.getAddress());
